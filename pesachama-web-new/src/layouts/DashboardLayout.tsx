...
export default function DashboardLayout() {
  ...
  async function getProfile() {
    try {
      ...
      // KYC Enforcement — only force onboarding if never started
  ...
  // Onboarding routes where we want a full-page experience without sidebar/nav
  const onboardingRoutes = ['/onboarding', '/verify-otp', '/membership-kyc']
  ...
  return (
    <div className={`flex h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 selection:bg-[#00C853]/30 ${isOnboarding ? 'overflow-auto' : ''}`}>
            ...
            <Link to="/" className="flex items-center gap-3 overflow-hidden">
              {sidebarOpen ? (
                <>
                  ...
                  <RatibuLogoDark className="h-12 w-auto" />
                </>
              ) : (
      ...
      {!isOnboarding && (
        <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-md dark:bg-slate-900/80 dark:backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 flex items-center justify-around px-2 z-20 md:hidden">
