-- Fix transaction PIN hashing so Postgres resolves digest() using bytea input explicitly.
...
BEGIN
  ...
  IF action IN ('set', 'reset') THEN
    ...
    pin_hash := encode(digest(convert_to(salt || ':' || pin, 'UTF8'), 'sha256'), 'hex');
  ...
  IF action = 'verify' THEN
    ...
    pin_hash := encode(digest(convert_to(user_row.transaction_pin_salt || ':' || pin, 'UTF8'), 'sha256'), 'hex');
